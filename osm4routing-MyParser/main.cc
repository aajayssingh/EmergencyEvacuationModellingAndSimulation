...
//ofstream myfile;
 ofstream  myfile("worldobjects.csv");
 //ofstream myfileosm("modifiedosm.osm");
 
double myLAT = 0, myLON=0, myID =0;
bool myflag = 0;
double myHEIGHT=0;
int ajheight = 0;
    ...
    else if(strcmp(el, "tag") == 0)
    {
        ...
        while (*attr != NULL)
        {
           ...
           /*     if(strcmp(key, "station")==0)
                    myHEIGHT  = 100;
                else  if(strcmp(key, "residential")==0)
                    myHEIGHT = 400;*/
            }
            else if ( strcmp(name, "v") == 0 )
            {
                ep.update(key, value, &ajheight);
              myfile <<myID<<";"<< key<<";"<<value;//@J
               //myfile<<100<<";"<<200;
                if(myflag)
                {
                    int h = *(&ajheight);
                 myfile<<";"<<setprecision(10)<<myLAT<<";"<<myLON<<";"<<h;//<<"\n";
                 // fprintf(&myfile, "%d",1);
                  myflag =0;
                //fprintf("%d   %d", myLAT, myLON);
                }
                //myfile<<"\t\t"<<myHEIGHT;
            }
        }
...
main(int argc, char** argv)
{
    ...
    string csv_nodes, csv_edges, csv_aj;
    po::options_description csv("CSV output options");
    csv.add_options()
   //   ("csv_aj", po::value<string>(&csv_aj)->default_value("aj.csv"), "csv output file for the aj")
    ...
    //==================== STEP 1 =======================//
// myfile.open ("Worldobjects.txt");//@J
 
 myfile<<"NODEID"<<";"<<"KEY"<<"\t"<<"VALUE"<<"\t"<<"LAT"<<"\t"<<"LON"<<"\t"<<"HEIGHT"<<"\n";
 //myfile<<"KEY"<<setw(15)<<"VALUE"<<setw(15)<<"LAT"<<setw(15)<<"LON"<<setw(15)<<"HEIGHT"<<"\n";
